// Isolated game snapshot for the turn-cue comparison.
'use strict';
(() => {
  const E = HeartsEngine, I = HeartsInsights, L = HeartsText, $ = id => document.getElementById(id);
  const KEY = 'codex-hearts-turn-option-' + document.documentElement.dataset.option, BACKUP = KEY + '-backup', PREFS = KEY + '-settings';
  // Every preview has its own save; replay never touches the real game.
  localStorage.removeItem(BACKUP);
  localStorage.setItem(KEY,JSON.stringify({version:2,game:{"version": 1, "handNumber": 1, "scores": [0, 0, 0, 0], "passOffset": 1, "phase": "play", "hands": [["5D", "10D", "4S", "10S", "3H", "5H", "6H", "7H", "8H", "9H", "JH"], ["6C", "10C", "JC", "QC", "4D", "2S", "5S", "JS", "QS", "2H", "QH"], ["5C", "3D", "6D", "7D", "8D", "AD", "3S", "8S", "4H", "10H", "AH"], ["4C", "7C", "8C", "9C", "AC", "6S", "7S", "9S", "KS", "AS", "KH"]], "received": [], "trick": [], "history": [{"cards": [{"player": 3, "card": "2C"}, {"player": 0, "card": "KD"}, {"player": 1, "card": "3C"}, {"player": 2, "card": "KC"}], "winner": 2, "points": 0}, {"cards": [{"player": 2, "card": "2D"}, {"player": 3, "card": "QD"}, {"player": 0, "card": "JD"}, {"player": 1, "card": "9D"}], "winner": 3, "points": 0}], "handPoints": [0, 0, 0, 0], "heartsBroken": false, "turn": 3, "result": null},selected:[]}));
  ...
  function renderGuidance() {
    ...
    if (!ownTurn) TurnCuePreview.leave();
    else if (!wasYourTurn) TurnCuePreview.enter();
    wasYourTurn = ownTurn;
