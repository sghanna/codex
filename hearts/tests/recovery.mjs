...
try {
 for(const lang of ['en','es','vi']) {
  ...
  await page.addInitScript(({KEY,state,lang})=>{localStorage.setItem(KEY,JSON.stringify({version:2,game:state,selected:[]}));localStorage.setItem('codex-hearts-settings-v2',JSON.stringify({language:lang,pace:'slow',sound:false}));},{KEY,state:fixtures.received,lang});
  await page.goto(url);
