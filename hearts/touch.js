...
const HeartsTouch = (() => {
  function install(root, context) {
    let press = null, suppressClick = false;
    const pointers = new Set();
    const unavailable = () => document.hidden || document.querySelector('dialog[open]');
    const targetButton = target => target instanceof Element ? target.closest('#hand .card, #primary-action') : null;
    function cancel() {
      if (!press) return;
